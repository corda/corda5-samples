...
import com.r3.developers.advanceCustomQuery.workflows.ChatStateResults;
...
public class ListAllMsgsWithHello implements ClientStartableFlow {
    ...
    public String call(ClientRequestBody requestBody) {
        ...
        PagedQuery.ResultSet<StateAndRef> resultSet = utxoLedgerService.query("GET_MSGS_CONTAINING_HELLO", StateAndRef.class)
                .setCreatedTimestampLimit(Instant.now()).setLimit(1000)
                .execute();

        List<ChatState> resultState = resultSet.getResults().stream().map(
                it -> (ChatState) it.getState().getContractState())
                .collect(Collectors.toList());

        List<ChatStateResults> results = resultState.stream().map(it ->
                new ChatStateResults(
                        it.getId(),
                        it.getChatName(),
                        it.getMessageFrom().toString(),
                        it.getMessage()
                )
        ).collect(Collectors.toList());
