...
import com.r3.developers.samples.fx.services.PrimeService
...
@InitiatingFlow(protocol = "create-fx-transaction")
class CreateForeignExchangeTransaction(): ClientStartableFlow {

    private data class CreateForeignExchangeTransaction(
    ...
    private companion object {
        ...
        const val CALLED = "CreateForeignExchangeTransaction.call() called."
        const val SET_UP = "Initialising flow variables."
        ...
        val primeServiceName: MemberX500Name = MemberX500Name.parse("CN=ForeignExchangeService, OU=Test Dept, O=R3, L=London, C=GB") //todo rename all to foreign exchange service
        val bobMemberNodeName: MemberX500Name = MemberX500Name.parse("CN=Bob, OU=Test Dept, O=R3, L=London, C=GB")
    }
    ...
    override fun call(requestBody: ClientRequestBody): String {
        ...
        log.info(SET_UP)
        val request = requestBody.getRequestBodyAs(jsonMarshallingService, CreateForeignExchangeTransaction::class.java)
        val convertingFrom = request.convertingFrom
        val convertingTo = request.convertingTo
        val amount = request.amount

        return "hey test test. convertingFrom:$convertingFrom convertingTo:$convertingTo amount:$amount"
...
//        val bobMemberNodeInfo = memberLookup.lookup(bobMemberNodeName) ?: throw IllegalArgumentException("Member '$bobMemberNodeName' not found on network.")
//        val ourIdentity = memberLookup.myInfo().ledgerKeys.first()
//        val primeServiceIdentity = primeServiceInfo.ledgerKeys.first()
//        val bobIdentity = bobMemberNodeInfo.ledgerKeys.first()
//
...
//        val primeOutputState = Prime(index, nthPrimeResult,listOf(ourIdentity,bobIdentity))
...
//            .setTimeWindowUntil(Instant.now().plus(1,ChronoUnit.DAYS))
//            .addSignatories(listOf(ourIdentity,bobIdentity))
//
//        log.info(WE_SIGN)
